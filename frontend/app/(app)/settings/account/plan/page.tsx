...
export default function AccountPlanPage() {
  ...
  // Mostra a acao (ativar Free / assinar pago) pro plano REGISTRADO na
  // assinatura (escolhido no onboarding, ou trocado depois) — nao so o
  // primeiro item da lista, que podia mostrar o formulario errado quando
  // havia mais de um plano ativo.
  const plan = plansQuery.data?.find((candidate) => candidate.id === subscription?.planId);
  const isFreePlan = plan?.priceAmount === 0;

  if (!isOwner) {
    return (
      <p className="text-muted-foreground text-sm">
        Somente o administrador da conta pode ver e gerenciar a assinatura.
      </p>
    );
  }
      ...
      {subscriptionQuery.isLoading ? (
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            {subscription.status === "Active" && subscription.canceledAtUtc && subscription.currentPeriodEndsAtUtc && (
              <CardDescription>
                Cancelamento agendado — voce continua com acesso ate {formatDate(subscription.currentPeriodEndsAtUtc)}{" "}
                (periodo ja pago).
              </CardDescription>
            )}
          </CardHeader>
          <CardContent className="space-y-3">
            {plan && (plan.maxUnits !== null || plan.maxProfessionals !== null || plan.maxCustomers !== null) && (
              <p className="text-muted-foreground text-xs">
                {[
                  plan.maxUnits !== null ? `${plan.maxUnits} unidade${plan.maxUnits === 1 ? "" : "s"}` : "unidades ilimitadas",
                  plan.maxProfessionals !== null ? `${plan.maxProfessionals} profissionais` : "profissionais ilimitados",
                  plan.maxCustomers !== null ? `${plan.maxCustomers.toLocaleString("pt-BR")} clientes` : "clientes ilimitados",
                ].join(" · ")}
              </p>
            )}
            {subscription.latestPayment && (
              <div className="text-sm">
                ...
                {subscription.latestPayment.invoiceUrl && subscription.latestPayment.status !== "Confirmed" ? (
                  ...
                  // Sem invoiceUrl e status != Confirmed: o backend so libera o
                  // link a partir de 10 dias antes do vencimento (ver
                  // GetMySubscriptionQueryHandler) -- avisa a data em vez de so
                  // sumir com o link sem explicacao.
                  subscription.latestPayment.status !== "Confirmed" && (
                    <p className="text-muted-foreground text-xs">
                      A fatura fica disponível a partir de{" "}
                      {new Date(
                        new Date(subscription.latestPayment.dueDate + "T00:00:00").getTime() - 10 * 24 * 60 * 60 * 1000
            ...
            {subscription.status === "Active" && !subscription.canceledAtUtc ? (
              <Button
                variant="outline"
      ...
      <AlertDialog open={confirmingCancel} onOpenChange={setConfirmingCancel}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {subscription &&
                (subscription.currentPeriodEndsAtUtc && new Date(subscription.currentPeriodEndsAtUtc) > new Date()
                  ? `Voce continua com acesso ao plano "${subscription.planName}" ate ${formatDate(subscription.currentPeriodEndsAtUtc)} — o periodo ja pago nao e perdido. Depois disso a assinatura sera cancelada de vez; para continuar usando um plano pago, sera preciso assinar novamente.`
